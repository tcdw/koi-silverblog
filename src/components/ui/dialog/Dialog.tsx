    ...
    return (
        <Dialog.Root open={state.isOpen} onOpenChange={closeDialog}>
            <Dialog.Portal>
                ...
                <Dialog.Content
                    class={twMerge(
                        "fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl",
                ...
                >
                    <div class="flex flex-col gap-3">
                        <Dialog.Title class="text-lg font-medium leading-6 text-gray-900">{state.title}</Dialog.Title>
                        <Dialog.Description class="text-sm text-gray-500">{state.description}</Dialog.Description>
                    </div>
                    <div class="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 mt-5">
                        {state.type === "confirm" && (
                            <Dialog.CloseButton
                                onClick={handleCancel}
                                class="block rounded-md px-4 py-2 text-center text-sm hover:bg-primary-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary-600"
                            >
                                取消
                            </Dialog.CloseButton>
                        )}
